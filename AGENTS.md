...
`DESIGN.md` is authoritative. Log deviations in `DECISIONS.md` before implementation. Track the active phase in `PROGRESS.md`. Current work is the reopened two-agent Phase 0 on `codex/phase-0-two-agent`.
...
3. Injection uses tmux bracketed paste, echo-verification, one retry, and a single Enter. Never inject unless the target is idle.
...
- Keep the core count-aware, but do not build generalized N-agent scheduling in Phase 0.
...
- **Codex:** project-local `.codex/hooks.json` command handlers for lifecycle events; sessions under `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`; one-time `/hooks` trust review. Do not install or replace the user's global `notify` setting.
...
- Phase 0 assumes one live session per adapter kind in the managed cwd. Reuse requires an exact daemon/tmux config and load-time source-content fingerprint; teardown may accept a stale fingerprint only when the config directory still proves the same target.
...
- Phase 5 — Extensibility and optional human-layer integrations.
